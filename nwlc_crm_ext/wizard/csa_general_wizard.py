...
import base64
class CSAGeneralWizard(models.TransientModel):
    _name = "csageneral.agreement"
    ...
    change_fee = fields.Char(string="Fee")
    
    @api.model
    def default_get(self, fields_list):
        res = super(CSAGeneralWizard, self).default_get(fields_list)
        res['change_fee'] = "10.9"
        return res    
    ...
    def print_report(self):
        ...
        return self.env.ref('nwlc_crm_ext.action_csa_general_agreement_template').report_action(self.crm_lead_id)
    ...
    def send_report(self):
        ...
        report  = self.env['ir.actions.report']._render_qweb_pdf("nwlc_crm_ext.action_csa_general_agreement_template",self.crm_lead_id.id)
        if not report:
        ...
        attachment = self.env['ir.attachment'].create({
            'name': f"CSA General Agreement.pdf",
            'type': 'binary',
        ...
        template = self.env.ref('nwlc_crm_ext.email_template_csa_general_agreement')  # Replace with your template ID
        template.send_mail(
    ...
    def send_csa_general(self):
        if not self.crm_lead_id:
            raise UserError(_("No associated CRM lead found."))

        pdf_content, _ = self.env['ir.actions.report']._render_qweb_pdf(
            "nwlc_crm_ext.action_csa_general_agreement_template", 
            self.crm_lead_id.id
        )

        if not pdf_content:
            raise UserError(_("Could not generate CSA General Agreement."))

        template = self.env.ref('nwlc_crm_ext.csa_general_agreement_template', raise_if_not_found=False)
        attachment = self.env['ir.attachment'].create({
            'name': f"{self.crm_lead_id.name}_csa_general_Document.pdf",
            'type': 'binary',
        ...
        if not template:
            template = self.env['sign.template'].create({
                'name': 'CSA General Agreement Template',
        ...
        sign_request = self.env['sign.request'].create({
            ...
            'reference': f"CSA General Agreement Signature for {self.crm_lead_id.name}",
            'request_item_ids': [(0, 0, {
                'partner_id': self.crm_lead_id.full_name.id,
                'role_id': 1,
            })],
            'reference' : "CSA General Agreement.pdf",
            'subject' : "Signature Request - CSA General Agreement.pdf"
        })
