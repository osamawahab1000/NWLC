...
import base64
class SpanishCSAWizard(models.TransientModel):
    _name = "spanish.csa.agreement"
    ...
    def print_report_spanish(self):
        """Trigger the CRM Lead report printing."""
        self.ensure_one()  # Ensure only one wizard is active
        return self.env.ref('nwlc_crm_ext.action_spanish_csa_template').report_action(self.crm_lead_id)
    
    
    def send_report_spanish(self):
        ...
        report  = self.env['ir.actions.report']._render_qweb_pdf("nwlc_crm_ext.action_spanish_csa_template",self.crm_lead_id.id)
        if not report:
        ...
        attachment = self.env['ir.attachment'].create({
            # 'name': f"CRM_Lead_Report_{self.crm_lead_id.id}.pdf",
            'name': f"Spanish CSA Rev-{self.crm_lead_id.state_id.code}.pdf",
            'type': 'binary',
        ...
        template = self.env.ref('nwlc_crm_ext.email_template_spanish_csa_rev')  # Replace with your template ID
        template.send_mail(
    ...
    def send_csa_spanish(self):
        if not self.crm_lead_id:
            raise UserError(_("No associated CRM lead found."))

        pdf_content, _ = self.env['ir.actions.report']._render_qweb_pdf(
            "nwlc_crm_ext.action_spanish_csa_template", 
            self.crm_lead_id.id
        )

        if not pdf_content:
            raise UserError(_("Could not generate CSA Spanish Agreement."))

        template = self.env.ref('nwlc_crm_ext.spanish_csa_template', raise_if_not_found=False)
        attachment = self.env['ir.attachment'].create({
            'name': f"{self.crm_lead_id.name}_csa_spanish_Document.pdf",
            'type': 'binary',
        ...
        if not template:
            template = self.env['sign.template'].create({
                'name': 'CSA Spanish Agreement Template',
        ...
        sign_request = self.env['sign.request'].create({
            ...
            'reference': f"CSA Spanish Agreement Signature for {self.crm_lead_id.name}",
            'request_item_ids': [(0, 0, {
                'partner_id': self.crm_lead_id.full_name.id,
                'role_id': 1,
            })],
            'reference' : "CSA Spanish Agreement.pdf",
            'subject' : "Signature Request - CSA Spanish Agreement.pdf"
        })
